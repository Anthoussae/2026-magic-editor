...
Additional review passes for this repo (apply when the diff touches the relevant area):

- **Licensing**: every new source file has the correct SPDX header — GPL-3.0-or-later + `Ported from MagicSetEditor2 ...` provenance note for ported code; AGPL-3.0-only for original code. Ported/original boundaries respected (no ported logic slipped into AGPL files or vice versa). `NOTICE` still accurate. Header violations are `P1`.
- **Asset/IP policy**: no third-party template assets, fonts, or artwork committed; no download/fetch logic for third-party assets in code; committed assets appear in `assets/ATTRIBUTION.md`. Violations are `P0` (they taint the repo's legal posture).
- **Engine purity**: `mse-*` crates gained no dioxus/web-sys/wasm-only/app dependencies; tests remain host-native. Violations are `P1`.
- **Stories**: new or visibly-changed UI components have stories covering main states (empty, error, long-content). Missing stories are `P2`.
- **Port fidelity**: changes to ported code preserve MSE2-compatible behavior; deliberate deviations are documented (crate README quirks list) and covered by conformance/golden tests. Undocumented behavior deviations are `P1`.
- **Teaching**: docs (`docs/dev/`, crate READMEs) updated when the change alters a workflow or introduces a concept; written at a level a newer coder can follow. Gaps are `P3`.
