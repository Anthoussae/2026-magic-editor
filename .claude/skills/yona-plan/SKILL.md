...
- **Active roadmap**: `docs/plans/2026-07-17-vertical-slice-roadmap/plan.md` (milestones M1–M10). New plans for milestone work must link the milestone file they execute (M6, M7, M8 explicitly require their own `yona-plan` run — start from their stub files).
- **ADRs**: `docs/adr/YYYY-MM-DD-short-title.md` (date-based, matching yona-implement).
- **Audience**: this repo teaches James (new coder). Plans must be readable by a newer coder: spell out commands, link reference docs (`docs/mse2/`, `docs/dev/`), and include a "Teaching goal" section when a milestone introduces a new concept or workflow.
- **Validation baseline**: `just ci` (fmt, clippy, tests, wasm build) once M1 lands; name any additional commands explicitly.
